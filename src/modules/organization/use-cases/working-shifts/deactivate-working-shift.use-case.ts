...
@Injectable()
export class DeactivateWorkingShiftUseCase {
  constructor(
  ...
  async execute(id: number, actorId: bigint) {
    ...
    if (!shift.is_active) {
      throw new BadRequestException(
        `Working shift ${id} is already deactivated.`,
      );
    }

    const result = await this.repo.softDelete(shiftId);

    void this.auditService.log({
      eventType: 'WORKING_SHIFT_DEACTIVATED',
